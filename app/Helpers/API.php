...
class API
{
    ...
    public function marshallEntry($item, $type)
    {
        ...
        $outer_relations = array_filter($relations, function ($el) {
            return (strpos($el, 'related') !== 0);
        });

        // Prepare resource
        $document->data = $this->formatResource($item, $type, $relations);

        if (count($outer_relations) > 0) {
            ...
            if ($isRelation) {
                ...
                if (count($collection) > 0) {
                    ...
                    foreach ($collection as $relatedItem) {
                        ...
                        $identifier->type = strpos($propName, 'related') === 0 ? $type : str_singular($propName);
    ...
    private function buildIncluded(&$document, $item, $relations, &$acc)
    {
        foreach ($relations as $relationName) {
            ...
            if (strpos($relationName, 'related') !== 0) {
